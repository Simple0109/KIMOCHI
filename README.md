...
① ユーザーAが報酬を得ようとしたとき、ユーザーBに「自分の得たい報酬」と「報酬得るための対価」を提示します
② ユーザーBはその「報酬」と「対価」を確認し精査を行います
③-1 ユーザーBがユーザーAの提示した報酬と対価を認め承認する場合
→ ユーザーAが報酬を得るために対価を払うことをユーザーBが許可します → ユーザーBも許可するアクションを実行します
③-2 ユーザーBがユーザーAの示した報酬と対価を承認できない場合
→ ユーザーBはユーザーAに「報酬はhogehogeに、対価はfugafugaとし修正しました(or修正してください)」とコメントし、再度、報酬と対価の調整を行います
→ グループXに所属する全てのユーザーが承認する報酬と対価になるまでこれを繰り返し、③-1に遷移します
④ グループXに所属するユーザー全員がユーザーAの報酬と対価を承認するとユーザーAは対価を払うことが可能になります
⑤ ユーザーAは対価全て支払い終わったら報酬を得ることができます
